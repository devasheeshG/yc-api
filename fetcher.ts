...
const githubRepo = "devasheeshG/yc-api";
const [repoOwner, repoName] = githubRepo.split("/");

const apiBaseUrl = `https://${repoOwner.toLowerCase()}.github.io/${repoName}`;
const readmeUrl = `https://github.com/${githubRepo}`;
...
const fetchAllCompanies = async (): Promise<LaunchedCompany[]> => {
    const algoliaApplicationId = mustEnv("ALGOLIA_APPLICATION_ID");
    const algoliaApiKey = mustEnv("ALGOLIA_API_KEY");
    const baseUrl = mustEnv("ALGOLIA_BASE_URL");
    const indexName = mustEnv("ALGOLIA_INDEX_NAME");
...
const results = companies.map((hit) => {
    ...
    return {
        ...
        api: `${apiBaseUrl}/batches/${slugify(
...
> = {
    companies: {
        all: {
            ...
            api: `${apiBaseUrl}/companies/all.json`,
        },
    },
    tags: {},
    industries: {},
...
const uniqueIndustries = Array.from(
    ...
    .map((industry) => {
        ...
        return {
            name: industry,
            slug,
            api: `${apiBaseUrl}/industries/${slug}.json`,
        };
    })
    .sort((a, b) => a.name.localeCompare(b.name));
...
const uniqueBatches = Array.from(new Set(results.map((result) => result.batch)))
    .map((batch) => {
        ...
        return {
            name: batch,
            slug,
            api: `${apiBaseUrl}/batches/${slug}.json`,
        };
    })
    .sort((a, b) => {
...
for (const { key, slug, name } of [
    ...
    meta.companies[slug] = {
        name,
        count: filteredResults.length,
        api: `${apiBaseUrl}/companies/${slug}.json`,
    };
}
...
const existingMeta = JSON.parse(
    await Deno.readTextFile("meta.json"),
...
if (hasChanges) {
    ...
    text += `\n## ℹ️ Metadata\n\n`;
    text += `API endpoint: ${apiBaseUrl}/meta.json\n\n`;
